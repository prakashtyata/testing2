...
if ( ! class_exists( 'YITH_Request_Quote_Premium' ) ) {
	...
	class YITH_Request_Quote_Premium extends YITH_Request_Quote {
		...
		public function __construct() {
			...
			if ( 'yes' == get_option( 'ywraq_enable_pdf', 'yes' ) && ( defined( 'DOING_CRON' ) || $this->is_admin() || ( isset( $_REQUEST['ywraq_checkout_quote'] ) && $_REQUEST['ywraq_checkout_quote'] == 'true' ) ) ) {
		...
		public function add_button_shop() {
			...
			$type_in_loop = apply_filters( 'yith_ywraq_show_button_in_loop_product_type', array( 'simple', 'subscription', 'external', 'yith-composite' ) );
		 ...
		 */
		function get_exclusion_value( $value, $post_id, $meta_key, $single ) {
			...
			if ( '_ywraq_hide_quote_button' === $meta_key && 'product' === get_post_type( $post_id ) ) {
				...
				$value = in_array( $post_id, $exclusion_prod );
			}
		...
		public function save_metabox_info( $post_id, $post ) {
			...
			if ( ( $post && $post->post_type == 'product' ) ) {
				$exclusion_prod = explode( ',', get_option( 'yith-ywraq-exclusions-prod-list', '' ) );
				$exclusion_prod = array_map( 'absint', $exclusion_prod );
				if ( ! isset( $_REQUEST['yit_metaboxes']['_ywraq_hide_quote_button'] ) ) {
					if ( in_array( $post_id, $exclusion_prod ) ) {
		...
		public function ajax_refresh_number_items() {
			...
			$atts = array(
				...
				'item_plural_name' => $_POST['item_plural_name']
			);
		 ...
		 */
		function send_the_quote_automatically( $raq, $order ) {
		...
		public function pdf_footer( $order_id ) {
			...
			wc_get_template( 'pdf/quote-footer.php', array( 'footer' => $footer_content, 'pagination' => $show_pagination, 'order_id' => $order_id ), '', YITH_YWRAQ_TEMPLATE_PATH . '/' );
		}
