...
if ( ! class_exists( 'YITH_Request_Quote' ) ) {
	...
	class YITH_Request_Quote {
		...
		public function __construct() {
			...
			/* plugin */
			if ( ! isset( $_REQUEST['action'] ) || $_REQUEST['action'] != 'yith_ywraq_action' || ywraq_yit_contact_form_installed() ) {
		...
		public function add_raq_button_to_block( $render, $data, $product ) {
			if ( get_option( 'ywraq_show_btn_woocommerce_blocks' ) == 'yes' ) {
				$data->raq = do_shortcode( '[yith_ywraq_button_quote product="' . $product->get_id() . '"]' );
		...
		private function _plugin_integrations() {
			...
			if ( class_exists( 'WC_Subscriptions' ) ) {
				...
				add_filter( 'ywraq_quote_subtotal_item_plain', array(
					$this,
					'update_subtotal_item_price_plain'
				), 10, 3 );
			}
		...
		public function is_admin() {
			...
			$is_frontend_action = isset( $_REQUEST['action'] ) && in_array( $_REQUEST['action'], apply_filters( 'yith_ywraq_frontend_action_list', array(
						'flatsome_quickview',
						'ux_quickview',
			...
			if ( ! $is_admin && 'yes' == get_option( 'ywraq_automate_send_quote', 'no' ) && '0' == get_option( 'ywraq_cron_time' ) ) {
				$is_admin = ( isset( $_REQUEST['context'] ) && $_REQUEST['context'] == 'frontend' ) && ( isset( $_REQUEST['wc-ajax'] ) && $_REQUEST['wc-ajax'] == 'ywraq_submit_default_form' );
			}
		...
		function start_session() {
			...
			if ( ! isset( $_COOKIE['woocommerce_items_in_cart'] ) && ( ( isset( $_POST['ywraq_action'] ) && 'add_item' == $_POST['ywraq_action'] ) ) || isset( $_REQUEST['add-to-quote'] ) ) {
				do_action( 'woocommerce_set_cart_cookies', true );
			}

			$cookie = ywraq_get_cookie_name() . '_' . COOKIEHASH;

			if ( ( isset( $_POST['ywraq_action'] ) && 'add_item' == $_POST['ywraq_action'] ) || isset( $_COOKIE[ $cookie ] ) || isset( $_REQUEST['add-to-quote'] ) || apply_filters( 'ywraq_force_start_session', false ) ) {
		...
		private function set_rqa_cookies( $set = true ) {
			...
			if ( $set ) {
				...
				wc_setcookie( $hash_name, md5( json_encode( $this->raq_content ) ) );
		...
		public function ajax_add_item() {
			...
			if ( ! empty( $wapo_valid ) ) {
				wp_send_json( apply_filters( 'yith_ywraq_ajax_add_item_json', array(
					'result'  => 'false',
		...
		public function add_to_quote_action() {
			...
			if ( empty( $_REQUEST['add-to-quote'] ) ) {
				return;
			}
		...
		public function send_message() {
			...
			if ( ! isset( $_REQUEST['raq_mail_wpnonce'] ) ) {
				return;
			}
			...
			if ( isset( $_POST['raq_mail_wpnonce'] ) ) {
				...
				if ( empty( $errors ) ) {
					$args = array(
						...
						'user_password'           => isset( $_POST['rqa_password'] ) ? $_POST['rqa_password'] : '',
					...
					} elseif ( isset( $_POST['createaccount'] ) || apply_filters( 'ywraq_force_create_account', false ) ) {
