...
if ( ! class_exists( 'YITH_YWRAQ_Exclusions_Handler' ) ) {
	...
	class YITH_YWRAQ_Exclusions_Handler {
		...
		public function save_exclusions_prod() {
			...
			if ( ! isset( $_POST['_nonce'] ) || ! wp_verify_nonce( $_POST['_nonce'], 'yith_ywraq_add_exclusions_prod' ) || ! isset( $_POST['add_products'] ) ) {
				return;
			}
		...
		public function delete_exclusion_prod() {
			...
			if ( ! isset( $_GET['remove_nonce'] ) || ! wp_verify_nonce( $_GET['remove_nonce'], 'yith_ywraq_remove_exclusions_prod' ) || ! isset( $_GET['remove_prod_exclusion'] ) ) {
				return;
			}
		...
		public function search_products_ajax() {
			...
			if ( is_numeric( $term ) ) {
				...
				$args2 = array( 'post_type'      => $post_types,
				                'post_status'    => 'publish',
				                'posts_per_page' => - 1,
				                'post__in'       => array( 0, $term ),
				                'fields'         => 'ids' );

				$args3 = array( 'post_type'      => $post_types,
				                'post_status'    => 'publish',
				                'posts_per_page' => - 1,
				                'post_parent'    => $term,
				                'fields'         => 'ids' );

				$args4 = array( 'post_type'      => $post_types,
				                'post_status'    => 'publish',
				                'posts_per_page' => - 1,
		...
		public function save_exclusions_cat() {
			...
			if ( ! isset( $_POST['_nonce'] ) || ! wp_verify_nonce( $_POST['_nonce'], 'yith_ywraq_add_exclusions_cat' ) || ! isset( $_POST['add_categories'] ) ) {
				return;
			}
		...
		public function delete_exclusion_cat() {
			...
			if ( ! isset( $_GET['remove_nonce'] ) || ! wp_verify_nonce( $_GET['remove_nonce'], 'yith_ywraq_remove_exclusions_cat' ) || ! isset( $_GET['remove_cat_exclusion'] ) ) {
				return;
			}
		...
		public function save_exclusions_tag() {
			...
			if ( ! isset( $_POST['_nonce'] ) || ! wp_verify_nonce( $_POST['_nonce'], 'yith_ywraq_add_exclusions_tag' ) || ! isset( $_POST['add_tags'] ) ) {
				return;
			}
			...
			// get new items
			$new_items = $_POST['add_tags'];
			! is_array( $new_items ) && $new_items = explode( ',', $_POST['add_tags'] );
			$new_items = array_filter( $new_items );
		...
		public function delete_exclusion_tag() {
			...
			if ( ! isset( $_GET['remove_nonce'] ) || ! wp_verify_nonce( $_GET['remove_nonce'], 'yith_ywraq_remove_exclusions_tag' ) || ! isset( $_GET['remove_tag_exclusion'] ) ) {
				return;
			}

			$exclusions = array_filter( explode( ',', get_option( 'yith-ywraq-exclusions-tag-list' ) ) );

			if ( ( $key = array_search( $_GET['remove_tag_exclusion'], $exclusions ) ) !== false ) {
				unset( $exclusions[ $key ] );
			}
