...
class YITH_YWRAQ_Order_Request {
			...
			if ( $cart_coupons ) {
				foreach ( $cart_coupons as $cart_coupon ) {
					if ( $cart_coupon == $coupon->get_code() ) {
						wc_add_notice( __( 'The discount has been removed because you have changed the quote contents.', 'yith-woocommerce-request-a-quote' ), 'error' );
	...
	public function save_order_as_quote( $order_id ) {
		...
		if ( isset( $_REQUEST['ywraq_checkout_quote'] ) && $_REQUEST['ywraq_checkout_quote'] == 'true' ) {
			...
			if ( ! is_ajax() ) {
				wp_safe_redirect(
					apply_filters( 'woocommerce_checkout_no_payment_needed_redirect', YITH_Request_Quote()->get_redirect_page_url( array( 'hidem' => 1, 'order' => $order->get_id() ) ), $order )
				);
				exit;
			}
			...
			wp_send_json(
				array(
					'result'   => 'success',
					'redirect' => apply_filters( 'woocommerce_checkout_no_payment_needed_redirect', YITH_Request_Quote()->get_redirect_page_url( array( 'hidem' => 1, 'order' => $order->get_id() ) ), $order ),
				)
			);
			exit;
		}
	...
	public function save_quote( $order ) {
		...
		if ( ! isset( $_POST['yit_metaboxes'] ) || ! isset( $_POST['yit_metaboxes']['_ywraq_safe_submit_field'] ) || empty( $_POST['yit_metaboxes']['ywraq_customer_email'] ) ) {
			return;
		}
	...
	public function save_new_quote( $order ) {
		...
		if ( empty( $_REQUEST['yit_metaboxes'] ['ywraq_customer_name'] ) || empty( $_REQUEST['yit_metaboxes']['ywraq_customer_email'] ) ) {
			return;
		}
	...
	public function set_cart_hash( $value ) {
		...
		if ( $order_id && $this->is_quote( $order_id ) ) {
			$hash = method_exists( WC()->cart, 'get_cart_hash' ) ? WC()->cart->get_cart_hash() : md5( json_encode( wc_clean( WC()->cart->get_cart_for_session() ) ) . WC()->cart->total );
	...
	public function load_view_quote_page_title( $title ) {
		...
		return sprintf( __( 'Quote #%s', 'yith-woocommerce-request-a-quote' ), apply_filters( 'ywraq_quote_number', $order_id ) );
	}
	...
	public function view_quote() {
		...
		if ( ! is_user_logged_in() ) {
			...
			wc_get_template( 'myaccount/view-quote.php',
			                 ...
			                 ), false, YITH_YWRAQ_TEMPLATE_PATH . '/' );
		}
	}
	...
	public function create_order( $raq ) {
		...
		if ( empty( $raq_content ) || get_option( 'ywraq_enable_order_creation', 'yes' ) != 'yes' ) {
			return false;
		}
		...
		if ( $attr ) {
			foreach ( $attr as $key => $item ) {
				...
				if ( is_callable( array( $order, $function ) ) ) {
					$order->$function( $item );
				} elseif ( $key == '_order_comments' && $item != '' ) {
	...
	public function add_metabox() {
		...
		$is_new_quote = isset( $_GET['new_quote'] ) && $_GET['new_quote'];


		if ( ( $post && 'shop_order' == $post->post_type || ( isset( $_GET['post_type'] ) && 'shop_order' == $_GET['post_type'] ) ) && ( $is_new_quote || 'yes' == $is_quote ) ) {
	...
	public function show_add_new_quote( $view ) {
		...
		printf( __( '<p><a href="%s" class="page-title-action">Add a new Quote</a></p>', 'yith-woocommerce-request-a-quote' ), $link );
	...
	public function ajax_mail_sent_order_created() {
		...
		if ( get_option( 'ywraq_enable_order_creation', 'yes' ) == 'no' ) {
		...
		if ( ! $order_id ) {
			yith_ywraq_add_notice( __( 'An error occurred creating your request. Please try again.', 'yith-woocommerce-request-a-quote' ), 'error' );
			wp_send_json(
				array(
	...
	public function ajax_create_order( $mail_sent_order_created = true ) {
		...
		$args = apply_filters( 'ywraq_ajax_create_order_args', $args, $_POST );
	...
	public function change_order_status() {
		...
		if ( ! isset( $_REQUEST['raq_nonce'] ) || ! isset( $_REQUEST['status'] ) || ! isset( $_REQUEST['request_quote'] ) ) {
			return;
		}


		$status   = $_REQUEST['status'];
		$order_id = $_REQUEST['request_quote'];

		$order = wc_get_order( $_REQUEST['request_quote'] );
		if ( ! $order ) {
			return;
		}
		...
		if ( ! ywraq_verify_token( $_REQUEST['raq_nonce'], 'accept-request-quote', $order_id, $user_email ) && ! ywraq_verify_token( $_REQUEST['raq_nonce'], 'reject-request-quote', $order_id, $user_email ) ) {
		...
		if ( $status == 'accepted' && ywraq_verify_token( $_REQUEST['raq_nonce'], 'accept-request-quote', $order_id, $user_email ) ) {

			if ( isset( $_REQUEST['pay_for_order'] ) && $_REQUEST['pay_for_order'] && in_array( $current_status, array(
					'ywraq-pending',
					'pending',
					'ywraq-accepted'
				) ) ) {
			...
			if ( $current_status == 'ywraq-rejected' && $status == 'rejected' ) {
				$args['message'] = sprintf( apply_filters( 'ywraq_rejected_quote_message', __( 'Quote n. %d has been rejected', 'yith-woocommerce-request-a-quote' ) ), $order_id );
				...
				if ( ! isset( $_REQUEST['raq_confirm'] ) && ! isset( $_REQUEST['confirm'] ) && $status == 'rejected' && ywraq_verify_token( $_REQUEST['raq_nonce'], 'reject-request-quote', $order_id, $user_email ) ) {
					$args = array(
						'status'        => $status,
						'raq_nonce'     => $_REQUEST['raq_nonce'],
	...
	public function is_expired( $order_id ) {
		...
		if ( $current_status == 'ywraq-expired' ) {
			return true;
		}
	...
	public function cart_validation( $result, $product_id ) {
		...
		if ( $order_id && $this->is_quote( $order_id ) && 'no' == get_option( 'ywraq_allow_add_to_cart', 'no' ) ) {
			$result = false;
			wc_add_notice( __( 'It\'s not possible to add products to the cart since you have already accepted a quote.', 'yith-woocommerce-request-a-quote' ), 'error' );
		}
	...
	public function cart_update_validation( $result, $cart_item_key ) {
		...
		if ( $order_id && $this->is_quote( $order_id ) && 'no' == get_option( 'ywraq_allow_add_to_cart', 'no' ) ) {
			$result = false;
			wc_add_notice( __( 'It\'s not possible change the quantity since you have already accepted a quote.', 'yith-woocommerce-request-a-quote' ), 'error' );
		}
