...
if ( ! function_exists( 'yith_ywraq_get_roles' ) ) {
	...
	function yith_ywraq_get_roles() {
		...
		return array_merge( array( 'all' => __( 'All', 'yith-woocommerce-request-a-quote' ) ), $roles );
	}
}
...
if ( ! function_exists( 'ywraq_get_label' ) ) {
	...
	function ywraq_get_label( $key, $echo = false ) {
		...
		switch ( $key ) {
			case 'product_added';
				$label = $option ? $option : apply_filters( 'yith_ywraq_product_added_to_list_message', __( 'Product added to the list!', 'yith-woocommerce-request-a-quote' ) );
				break;
			case 'browse_list';
				$label = $option ? $option : apply_filters( 'ywraq_product_added_view_browse_list', __( 'Browse the list', 'yith-woocommerce-request-a-quote' ) );
				break;
			case 'btn_link_text';
				$label = $option ? $option : apply_filters( 'ywraq_product_add_to_quote', __( 'Add to Quote', 'yith-woocommerce-request-a-quote' ) );
				break;
			case 'already_in_quote';
				$label = $option ? $option : apply_filters( 'yith_ywraq_product_already_in_list_message', __( 'Product already in the list.', 'yith-woocommerce-request-a-quote' ) );
				break;
			case 'accept' :
...
if ( ! function_exists( 'yith_ywraq_print_notices' ) ) {
	...
	function yith_ywraq_print_notices() {
		...
		if ( ! $session ) {
			if ( isset( $_GET['order'] ) ) {
				$order                    = $_GET['order'];
				$all_notices['success'][] = ywraq_get_message_after_request_quote_sending( $order );
			} else {
				return;
			}
		} else {
...
if ( ! function_exists( 'ywraq_get_list_empty_message' ) ) {
	...
	function ywraq_get_list_empty_message() {
		...
		$empty_list_message_text = apply_filters( 'ywraq_get_list_empty_message_text', __( 'Your list is empty, add products to the list to send a request', 'yith-woocommerce-request-a-quote' ) );
